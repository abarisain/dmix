...
public class DNSQuestion extends DNSEntry {
    ...
    private static class AllRecords extends DNSQuestion {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            ...
            if (jmDNSImpl.getLocalHost().getName().equalsIgnoreCase(loname)) {
                ...
                answers.addAll(jmDNSImpl.getLocalHost().answers(this.isUnique(),
    ...
    private static class DNS4Address extends DNSQuestion {
        ...
        @Override
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            DNSRecord answer = jmDNSImpl.getLocalHost().getDNSAddressRecord(this.getRecordType(),
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            ...
            if (this.isServicesDiscoveryMetaQuery()) {
                for (String serviceType : jmDNSImpl.getServiceTypes().keySet()) {
                    ...
                    answers.add(new DNSRecord.Pointer("_services._dns-sd._udp.local.",
                    ...
                    if (ipValue.equalsIgnoreCase(hostIPAddress)) {
                        if (this.isV4ReverseLookup()) {
                            answers.add(jmDNSImpl.getLocalHost().getDNSReverseAddressRecord(
                                    DNSRecordType.TYPE_A, DNSRecordClass.NOT_UNIQUE,
                                    DNSConstants.DNS_TTL));
                        }
                        if (this.isV6ReverseLookup()) {
                            answers.add(jmDNSImpl.getLocalHost().getDNSReverseAddressRecord(
                                    DNSRecordType.TYPE_AAAA, DNSRecordClass.NOT_UNIQUE,
                                    DNSConstants.DNS_TTL));
                        }
                    }
                }
    ...
    private static class Text extends DNSQuestion {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            this.addAnswersForServiceInfo(jmDNSImpl, answers, (ServiceInfoImpl) jmDNSImpl
                    .getServices().get(this.getName().toLowerCase()));
        }
     ...
     * @param unique Request unicast response (Currently not supported in this
     *            implementation)
     * @return new question
     */
    ...
    protected void addAnswersForServiceInfo(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers,
        ...
        if ((info != null) && info.isAnnounced()) {
            if (this.getName().equalsIgnoreCase(info.getQualifiedName())
                ...
                answers.addAll(jmDNSImpl.getLocalHost().answers(DNSRecordClass.UNIQUE,
                        DNSConstants.DNS_TTL));
                answers.addAll(info.answers(DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL,
                        jmDNSImpl.getLocalHost()));
            }
            if (logger.isLoggable(Level.FINER)) {
                logger.finer(jmDNSImpl.getName() + " DNSQuestion(" + this.getName()
