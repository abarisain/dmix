...
public abstract class DNSRecord extends DNSEntry {
    ...
    public static abstract class Address extends DNSRecord {
        ...
        protected Address(String name, DNSRecordType type, DNSRecordClass recordClass,
                boolean unique, int ttl, byte[] rawAddress) {
        ...
        protected Address(String name, DNSRecordType type, DNSRecordClass recordClass,
                boolean unique, int ttl, InetAddress addr) {
            super(name, type, recordClass, unique, ttl);
            this._addr = addr;
        }

        @Override
        DNSOutgoing addAnswer(JmDNSImpl dns, DNSIncoming in, InetAddress addr, int port,
        ...
        public ServiceInfo getServiceInfo(boolean persistent) {
            ServiceInfoImpl info = new ServiceInfoImpl(this.getQualifiedNameMap(), 0, 0, 0,
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            DNSRecord.Address dnsAddress = dns.getLocalHost().getDNSAddressRecord(
                    this.getRecordType(), this.isUnique(), DNSConstants.DNS_TTL);
            if (dnsAddress != null) {
                if (dnsAddress.sameType(this) && dnsAddress.sameName(this)
                        && (!dnsAddress.sameValue(this))) {
                    logger1.finer("handleQuery() Conflicting probe detected. lex compare "
                            + compareTo(dnsAddress));
                    // Tie-breaker test
        ...
        protected void toString(StringBuilder aLog) {
            super.toString(aLog);
            aLog.append(" address: '"
                    + (this.getAddress() != null ? this.getAddress().getHostAddress() : "null")
                    + "'");
        }
    ...
    public static class Pointer extends DNSRecord {
        ...
        public Pointer(String name, DNSRecordClass recordClass, boolean unique, int ttl,
                String alias) {
        ...
        public ServiceInfo getServiceInfo(boolean persistent) {
            if (this.isServicesDiscoveryMetaQuery()) {
                ...
                Map<Fields, String> map = ServiceInfoImpl.decodeQualifiedNameMapForType(this
                        .getAlias());
                return new ServiceInfoImpl(map, 0, 0, 0, persistent, (byte[]) null);
            } else if (this.isReverseLookup()) {
                return new ServiceInfoImpl(this.getQualifiedNameMap(), 0, 0, 0, persistent,
                        (byte[]) null);
    ...
    public static class Service extends DNSRecord {
        ...
        public Service(String name, DNSRecordClass recordClass, boolean unique, int ttl,
                ...
                if (this._port == info.getPort() != _server.equals(dns.getLocalHost().getName())) {
                    return dns.addAnswer(in, addr, port, out,
                            new DNSRecord.Service(info.getQualifiedName(), DNSRecordClass.CLASS_IN,
                                    ...
                                    info.getPriority(), info.getWeight(), info.getPort(), dns
        ...
        @Override
        public ServiceInfo getServiceInfo(boolean persistent) {
            return new ServiceInfoImpl(this.getQualifiedNameMap(), _port, _weight, _priority,
                    persistent, _server);
        }
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            ...
            if (info != null
                    ...
                    && (_port != info.getPort() || !_server.equalsIgnoreCase(dns.getLocalHost()
                            .getName()))) {
                logger1.finer("handleQuery() Conflicting probe detected from: " + getRecordSource());
                DNSRecord.Service localService = new DNSRecord.Service(info.getQualifiedName(),
                        DNSRecordClass.CLASS_IN, DNSRecordClass.UNIQUE, DNSConstants.DNS_TTL,
                        info.getPriority(), info.getWeight(), info.getPort(), dns.getLocalHost()
                                .getName());
    ...
    public static class Text extends DNSRecord {
        ...
        protected void toString(StringBuilder aLog) {
            super.toString(aLog);
            aLog.append(" text: '"
                    + ((_text.length > 20) ? new String(_text, 0, 17) + "..." : new String(_text))
                    + "'");
        }
    ...
    public final static byte[] EMPTY_TXT = new byte[] {
        0
    };
     ...
     */
    abstract DNSOutgoing addAnswer(JmDNSImpl dns, DNSIncoming in, InetAddress addr, int port,
            DNSOutgoing out) throws IOException;
     ...
     * Return a service information associated with that record if appropriate.
     * 
     * @param persistent if <code>true</code> ServiceListener.resolveService
     *            will be called whenever new new information is received.
     * @return service information
     */
